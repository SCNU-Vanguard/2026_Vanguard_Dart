 ...
 *********************************************************/
#ifndef __CONTROL_STATE_H_ /* 按 __CONTROL_STATE_H_ 选择编译分支。 */
#define __CONTROL_STATE_H_ /* 定义 __CONTROL_STATE_H_。 */
...
#define DEBUG_MODE_ALWAYS_ALLOW 1 /* 定义 DEBUG_MODE_ALWAYS_ALLOW。 */
...
#define TRIGGER_MIN -10000.0f /* 定义 TRIGGER_MIN。 */
#define TRIGGER_MAX 0.0f /* 定义 TRIGGER_MAX。 */
#define ENERGY_LEFT_MIN (-910.0f) /* 定义 ENERGY_LEFT_MIN。 */
#define ENERGY_LEFT_MAX (0.0f) /* 定义 ENERGY_LEFT_MAX。 */
#define ENERGY_RIGHT_MIN (0.0f) /* 定义 ENERGY_RIGHT_MIN。 */
#define ENERGY_RIGHT_MAX (910.0f) /* 定义 ENERGY_RIGHT_MAX。 */
...
} ControlInput_t; /* 结束 ControlInput_t 类型定义。 */
...
} ControlState_t; /* 结束 ControlState_t 类型定义。 */
...
extern SemaphoreHandle_t g_xMotorCtrlSemHandle; /* 声明外部变量 g_xMotorCtrlSemHandle。 */
extern SemaphoreHandle_t g_xDebugFinishedSemHandle; /* 声明外部变量 g_xDebugFinishedSemHandle。 */

/*============================== 任务句柄声明 ==============================*/

extern osThreadId_t IbusTaskHandle; /* 声明外部变量 IbusTaskHandle。 */
extern osThreadId_t ControlTaskHandle; /* 声明外部变量 ControlTaskHandle。 */
 ...
 * @note 在RTOS启动前调用
 */
void ControlState_Init(void); /* 声明 ControlState_Init 接口。 */
void ControlState_WatchdogInit(void); /* 声明 ControlState_WatchdogInit 接口。 */
void ControlState_IbusLostCallback(SoftwareWatchdogId_e id); /* 声明 ControlState_IbusLostCallback 接口。 */

/**
 * @brief 创建IbusTask和ControlTask
 * @note 在TaskInitFunc()中调用
 */
void ControlState_CreateTasks(void); /* 声明 ControlState_CreateTasks 接口。 */

/**
 * @brief 检查是否处于手动覆盖模式
 * @return true-手动模式，false-正常模式
 */
bool ControlState_IsManualOverride(void); /* 声明 ControlState_IsManualOverride 接口。 */

/**
 * @brief 开启调试窗口（单发完成后调用）
 * @param duration_ms 调试窗口持续时间（毫秒）
 */
void ControlState_StartDebugWindow(uint32_t duration_ms); /* 声明 ControlState_StartDebugWindow 接口。 */
 ...
 */
static inline float ControlState_NormalizeStick(int16_t raw) /* 实现 ControlState_NormalizeStick。 */
{
    float norm = (float)(raw - STICK_CENTER) / (float)STICK_RANGE; /* 初始化 norm。 */

    // 限幅
    if (norm > 1.0f) /* 检查当前执行条件。 */
        norm = 1.0f; /* 更新 norm。 */
    else if (norm < -1.0f) /* 继续判断下一条件。 */
        norm = -1.0f; /* 更新 norm。 */

    // 死区处理
    if (norm > -STICK_DEADZONE && norm < STICK_DEADZONE) /* 检查当前执行条件。 */
    {
        norm = 0.0f; /* 更新 norm。 */
    }

    return norm; /* 返回当前计算结果。 */
}
 ...
 */
static inline int8_t ControlState_GetDirection(int16_t raw) /* 实现 ControlState_GetDirection。 */
{
    // 非回中摇杆：默认在底部(1000)，只有0和1两个状态
    // 超过阈值认为是激活状态
    if (raw > NON_CENTER_HIGH) /* 检查当前执行条件。 */
        return 1; // 激活（摇杆向上推）
    else /* 处理其余情况。 */
 ...
 * @return 限幅后的值
 */
static inline float ControlState_Clamp(float value, float min, float max) /* 实现 ControlState_Clamp。 */
{
