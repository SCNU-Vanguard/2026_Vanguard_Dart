...
{
    return CAN_SendData(&hcan1, hdr, data, CAN_TX_RETRY_ENABLE);
}
...
{
    /*
     * 测试工程精简版 - 仅注册 4 发 RM3508 储能测试所需电机:
     *   - 左右储能 M3508 (位置环 + 软限位 + S 型规划)
     *   - 扳机 M2006
     * 不注册夹爪 M3508、Yaw DM4310。
     * 同步 PID 已启用 (上层调度由调用方决定是否使用)。
     */

    RM_MotorConfig_t left_store_config = {
        .direction_bias = 0.0f,
        .position_min = STORE3508_LEFT_POS_MIN_DEG,
        .position_max = STORE3508_LEFT_POS_MAX_DEG,
        .position_tolerance = MOTOR_DEAD_ZONE,
        .reverse = 0U,
    };
    RM_MotorConfig_t right_store_config = {
        .direction_bias = 0.0f,
        .position_min = STORE3508_RIGHT_POS_MIN_DEG,
        .position_max = STORE3508_RIGHT_POS_MAX_DEG,
        .position_tolerance = MOTOR_DEAD_ZONE,
        .reverse = 0U,
    };
    ...
    MotorManager.registered_count = 3U;
