...
/* 双侧蓄力 3508 位置同步 PID 参数：上层生成 base 目标后，
 * 由 RM_Motor_UpdateStoreSync 把 (left_pos - right_pos) 推向 0。
 * 分发规则：LeftFinal = LeftTarget + correction；RightFinal = RightTarget - correction。
 * 参数起点（需实物整定）。 */
#define STORE3508_SYNC_PID_KP (0.50f)
#define STORE3508_SYNC_PID_KI (0.00f)
#define STORE3508_SYNC_PID_KD (0.00f)
#define STORE3508_SYNC_PID_KF (0.00f)
#define STORE3508_SYNC_PID_MAX_OUT (700.0f)
#define STORE3508_SYNC_PID_MIN_OUT (0.0f)
#define STORE3508_SYNC_PID_MAX_IOUT (200.0f)

// 储能电机相关
// TODO: 以下 6 组位置宏值量纲与 STORE3508_*_POS_*_DEG 软限位不一致，
//       LeftStoreLoad/Bottom/Safe 目前都是 -10，四个业务位置无法区分；
//       RightStoreBottom=1000 会被 STORE3508_RIGHT_POS_MAX_DEG=0 钳制；
//       LimitStore=930 与 LeftStoreTop/RightStoreTop=0 也不匹配。
//       需按实际编码器刻度重新标定，并和 STORE3508_*_POS_*_DEG 对齐到同一坐标系。
#define LeftStoreLoad (50.0f)     // 换弹位置
#define RightStoreLoad (50.0f)    // 换弹位置
...
#define LeftSafe (0.0f)           // 左侧滑台叉上滑顶部
#define RightSafe (0.0f)          // 右侧滑台叉上滑顶部
...
// TODO: 以下 STORE3508_*_POS_*_DEG 为储能 3508 的软限位，
//       当前值 (LEFT: [0, 18000], RIGHT: [-18000, 0]) 是从 Test_Load 同步过来的占位值，
//       与 Dart_Ctrl 业务代码使用的 LeftStoreLoad/Bottom/Top 等量纲不匹配，
//       需按实际机械行程和编码器刻度重新标定，并和上面那组位置宏对齐到同一坐标系。
#define STORE3508_LEFT_POS_MIN_DEG (0.0f)
#define STORE3508_LEFT_POS_MAX_DEG (18000.0f)
#define STORE3508_RIGHT_POS_MIN_DEG (-18000.0f)
#define STORE3508_RIGHT_POS_MAX_DEG (0.0f)

/* 发射数据确认来源 */
#define TRUST_REFEREE_DART_DATA 0U
#define TRUST_REMOTE_CONTROL_DART_DATA 1U
#define TRUST_RCandREFEREE_DART_DATA 2U
#define TRUST_SHOOT_DART_DATA TRUST_REFEREE_DART_DATA
...
// 每发循环各阶段时长 (毫秒)
#define TEST_STORE_CHARGE_MS 800U  // 储能上升到 max 的等待时长
#define TEST_STORE_RELEASE_MS 300U // 储能回到 min (释放) 的等待时长
#define TEST_INTER_SHOT_MS 500U    // 两发之间的停顿

// 每发循环次数
#define TEST_SHOT_COUNT 4U

// 储能 3508 测试工作位置 (与软限位同坐标系, 可按实际机械调节)
#define TEST_STORE_LEFT_CHARGE_DEG STORE3508_LEFT_POS_MAX_DEG
#define TEST_STORE_LEFT_RELEASE_DEG STORE3508_LEFT_POS_MIN_DEG
#define TEST_STORE_RIGHT_CHARGE_DEG STORE3508_RIGHT_POS_MIN_DEG
#define TEST_STORE_RIGHT_RELEASE_DEG STORE3508_RIGHT_POS_MAX_DEG

// 4 发循环控制周期
#define TEST_STORE_CTRL_PERIOD_MS 5U
